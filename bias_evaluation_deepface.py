...
if __name__ == "__main__":
    args = get_args()
    analyze_image(args)
    image_path_list = pd.read_csv(os.path.join(args.results_path, 'AR_Flux_20250508_202121.csv')).generated_image_url.values
    ref_path_list = pd.read_csv(os.path.join(args.results_path, 'AR_Flux_20250508_202121.csv')).generated_image_url.values
    for row in image_path_list:
        ...
        with open(os.path.join(args.results_path, 'deepface_analysis_FLUX.json'), "w") as f:
