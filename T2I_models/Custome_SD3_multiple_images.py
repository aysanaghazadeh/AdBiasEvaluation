...
class ProjectionBlock(torch.nn.Module):
    ...
    def forward(self, image, encoded_prompt, encoded_reason, encoded_cultural_components, time_step):
        ...
        inputs = self.CLIP_processor(images=image[0], return_tensors="pt").to(self.args.device)
        clip_image_features_1 = self.CLIP_model.get_image_features(**inputs)
        clip_image_features_1 = clip_image_features_1 / clip_image_features_1.norm(p=2, dim=1, keepdim=True)
        clip_image_features_1 = self.projection_layer(clip_image_features_1)
        clip_image_features_1 = clip_image_features_1.unsqueeze(1) 
        inputs = self.CLIP_processor(images=image[1], return_tensors="pt").to(self.args.device)
        clip_image_features_2 = self.CLIP_model.get_image_features(**inputs)
        clip_image_features_2 = clip_image_features_2 / clip_image_features_2.norm(p=2, dim=1, keepdim=True)
        clip_image_features_2 = self.projection_layer(clip_image_features_2)
        clip_image_features_2 = clip_image_features_2.unsqueeze(1) 
        inputs = self.CLIP_processor(images=image[2], return_tensors="pt").to(self.args.device)
        clip_image_features_3 = self.CLIP_model.get_image_features(**inputs)
        clip_image_features_3 = clip_image_features_3 / clip_image_features_3.norm(p=2, dim=1, keepdim=True)
        clip_image_features_3 = self.projection_layer(clip_image_features_3)
        clip_image_features_3 = clip_image_features_3.unsqueeze(1) 
        clip_image_features = torch.cat([clip_image_features_1, clip_image_features_2, clip_image_features_3], dim=1)
...
class CustomeSD3MultipleImages(nn.Module):
    def __init__(self, args):
        super(CustomeSD3MultipleImages, self).__init__()
    ...
    def forward(self, prompt):
        ...
        style_image_1 = Image.open(os.path.join(self.args.data_path, "train_images_total", style_images[0]))
        style_image_2 = Image.open(os.path.join(self.args.data_path, "train_images_total", style_images[1]))
        style_image_3 = Image.open(os.path.join(self.args.data_path, "train_images_total", style_images[2]))
        style_image = [style_image_1, style_image_2, style_image_3]
        negative_style_image_1 = Image.open(os.path.join(self.args.data_path, "train_images_total", negative_style_image))
        negative_style_image_2 = Image.open(os.path.join(self.args.data_path, "train_images_total", negative_style_image))
        negative_style_image_3 = Image.open(os.path.join(self.args.data_path, "train_images_total", negative_style_image))
        negative_style_image = [negative_style_image_1, negative_style_image_2, negative_style_image_3]
