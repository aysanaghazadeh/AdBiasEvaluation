...
class CustomeSD3(nn.Module):
    def __init__(self, args):
        ...
        if not args.train and args.fine_tuned:
            self.projection_block.load_state_dict(torch.load(os.path.join(args.model_path, "SD3_modified/checkpoint-7500/projection_block.pt")))
    ...
    def forward(self, prompt, topic=None):
        ...
        for image in style_images:
            if self.topics[image] in TOPIC_MAP:
                image_topic = TOPIC_MAP[self.topics[image]]
            else:
                image_topic = self.topics[image]
