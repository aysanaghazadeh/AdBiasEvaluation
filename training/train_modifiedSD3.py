...
def train(args):
    ...
    if args.lora_layers is not None:
        ...
        target_modules.append("projection_block.projection_layer")
        target_modules.append("projection_block.cross_attention")
    else:
        target_modules = [
            "attn.add_k_proj",
            "attn.add_q_proj",
    ...
    def save_model_hook(models, weights, output_dir):
        if accelerator.is_main_process:
            ...
            projection_block_state_dict = None
            ...
            for model in models:
                ...
                elif isinstance(unwrap_model(model), ProjectionBlock):
                    model = unwrap_model(model)
                    if args.upcast_before_saving:
                        model = model.to(torch.float32)
                    projection_block_state_dict = model.state_dict()
                else:
            ...
            # Save projection block state dict separately
            if projection_block_state_dict is not None:
                torch.save(projection_block_state_dict, os.path.join(output_dir, "projection_block.pt"))
    ...
    projection_block_parameters_with_lr = {"params": projection_block.parameters(), "lr": args.learning_rate}
    if args.train_text_encoder:
        ...
        params_to_optimize = [
            ...
            projection_block_parameters_with_lr,
            # text_lora_parameters_one_with_lr,
            # text_lora_parameters_two_with_lr,
        ]
    else:
    ...
    train_dataloader = torch.utils.data.DataLoader(
        ...
        shuffle=True,
        collate_fn=lambda examples: collate_fn_modified(examples, args.with_prior_preservation),
    ...
    if args.train_text_encoder:
        (
            ...
            transformer, text_encoder_one, text_encoder_two, optimizer, train_dataloader, lr_scheduler, projection_block
        )
        assert text_encoder_one is not None
        assert text_encoder_two is not None
        assert text_encoder_three is not None
        assert projection_block is not None
    else:
        transformer, optimizer, train_dataloader, lr_scheduler, projection_block = accelerator.prepare(
            transformer, optimizer, train_dataloader, lr_scheduler, projection_block
        )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                if train_dataset.custom_instance_prompts:
                    if not args.train_text_encoder:
                        ...
                        reason_embeds, reason_pooled_prompt_embeds = encode_prompt(
                            text_encoders=[text_encoder_one, text_encoder_two, text_encoder_three],
                            tokenizers=[None, None, None],
                            prompt=reasons,
                            max_sequence_length=args.max_sequence_length,
                        ...
                        cultural_embeds, cultural_pooled_prompt_embeds = encode_prompt(
                            text_encoders=[text_encoder_one, text_encoder_two, text_encoder_three],
                            tokenizers=[None, None, None],
                ...
                # negative_model_pred = transformer(
                #     hidden_states=noisy_model_input,
                #     timestep=timesteps,
                ...
                if args.precondition_outputs:
                    ...
                    # negative_model_pred = negative_model_pred * (-sigmas) + noisy_model_input
                ...
                if args.with_prior_preservation:
                    ...
                    negative_model_pred, negative_model_pred_prior = torch.chunk(negative_model_pred, 2, dim=0)
                    ...
                    # negative_prior_loss = torch.mean(
                    #     (weighting.float() * (negative_model_pred_prior.float() - target_prior.float()) ** 2).reshape(
                    #         target_prior.shape[0], -1
                    #     ),
                    #     1,
                    # )
                    prior_loss = prior_loss.mean()
                    # negative_prior_loss = negative_prior_loss.mean()
                ...
                #     (weighting.float() * (negative_model_pred.float() - target.float()) ** 2).reshape(target.shape[0], -1),
                #     1,
                # )
                loss = loss.mean()
                # negative_loss = negative_loss.mean()
                
                # # Implement contrastive loss with margin
                # margin = 1.0  # You can adjust this margin value
                # contrastive_loss = torch.max(torch.zeros_like(loss), loss - negative_loss + margin)
            ...
            if accelerator.is_main_process:
                if args.validation_prompts is not None and step % 200 == 0:
                    if not args.train_text_encoder:
                        # create pipeline
                        text_encoder_one, text_encoder_two, text_encoder_three = load_text_encoders(
                            args, text_encoder_cls_one, text_encoder_cls_two, text_encoder_cls_three
    ...
    if accelerator.is_main_process:
        ...
        if args.push_to_hub:
            save_model_card(
                ...
                validation_prompt=args.validation_prompts[0],
